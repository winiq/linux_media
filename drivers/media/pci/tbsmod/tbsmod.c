/*
 * PikoTV's Changes for ASI Part:
 * -------------------------------------------------------------------------------------------
 * If output bitrate is lower than or equal to 999 then the unit will be considered as Mbit.
 * So any value between 0-200 will work as is (normally).
 * Any bitrate over PIKOTV_MBKB_THRESHOLD will trigger Kbit patch and bitrates will be treated
 * as unit Kbit.
 * Note: Patched points are marked with 'PikoTV'.
 * 20200306
 */
...
static void start_dma_transfer(struct mod_channel *pchannel)
{
    ...
    if (pchannel->input_bitrate)
    {
        ...
        {
            // Kbit
            speedctrl = div_u64(1000000000ULL * BLOCKSIZE, (pchannel->input_bitrate) * 1024);
        }
        else
        {
            // Mbit
            speedctrl = div_u64(1000000000ULL * BLOCKSIZE, (pchannel->input_bitrate) * 1024*1024);
        }
...
void channelprocess(struct tbs_pcie_dev *dev,u8 index){
        ...
        else
        {
            ...
            {
                // Kbit
                delay = div_u64(1000000000ULL * BLOCKSIZE, (pchannel->input_bitrate) * 1024*3);
            }
            else
            {
                // Mbit
                        ...
                        const struct pci_device_id *pci_id)
{
    ...
    for (i=0; i < CHANNELS; i++)
    {
        ...
        if ((dev->cardid == 0x6004) || (dev->cardid == 0x6014))
            ...
            dev->channel[i].input_bitrate = 80; /* PikoTV Bitrate - add *1024 for Kbit */
